-- ============================================================
-- TRS — Unified Schema
-- CRM (12 tables) + WhatsApp AI Agent (3 tables) combined
--
-- Paste into: https://supabase.com/dashboard/project/ctjapwjpwkvxubdmzbqg
-- SQL Editor → New Query → Paste → Run and enable RLS
-- ============================================================
...
-- CONTACTS
-- 1 row = 1 person
-- Merged: CRM fields + WhatsApp agent fields
-- At least one of email or phone must be present
-- ============================================================
create table contacts (
  id          uuid        primary key default gen_random_uuid(),
  full_name   text,
  email       text        unique,                             -- unique when set
  phone       text        unique,                             -- unique when set (E.164, e.g. 6591234567)
  source      text        check (source in ('website', 'whatsapp', 'email', 'manual')),
  crm_data    jsonb       not null default '{}',              -- flexible AI-extracted data
...
-- contact_id nullable on arrival — matched later
-- ============================================================
...
-- One per active WhatsApp engagement (AI agent)
...
create table whatsapp_conversations (
  ...
  pending_question jsonb,                                       -- { field_name, question } AI waiting for answer
...
-- ============================================================
-- WHATSAPP MESSAGES
-- Unified: CRM raw log + AI agent message tracking
-- ============================================================
create table whatsapp_messages (
  ...
  message              text,
  media_url            text,
  whatsapp_message_id  text        unique,                      -- Meta message ID for deduplication
  sent_at              timestamptz,
...
-- ============================================================
-- WHATSAPP DRAFTS
-- AI-generated draft replies, one pending per conversation
-- Renamed from: drafts
-- ============================================================
...
-- whatsapp_messages
create index idx_wa_msg_contact              on whatsapp_messages       (contact_id);
create index idx_wa_msg_conversation         on whatsapp_messages       (conversation_id);
create index idx_wa_msg_phone                on whatsapp_messages       (phone_number);
create index idx_wa_msg_sent                 on whatsapp_messages       (sent_at desc);

-- whatsapp_drafts
create index idx_wa_drafts_conversation      on whatsapp_drafts         (conversation_id, status);
...
create trigger trg_wa_conv_updated           before update on whatsapp_conversations for each row execute function update_updated_at();
create trigger trg_wa_drafts_updated         before update on whatsapp_drafts        for each row execute function update_updated_at();


-- ============================================================
-- ROW LEVEL SECURITY (RLS)
-- Staff (authenticated) = full access
-- Anonymous (website visitors) = insert only on leads + whatsapp_messages
-- ============================================================
...
create policy "staff_wa_conversations"       on whatsapp_conversations for all using (auth.role() = 'authenticated');
create policy "staff_wa_messages"            on whatsapp_messages      for all using (auth.role() = 'authenticated');
create policy "staff_wa_drafts"              on whatsapp_drafts        for all using (auth.role() = 'authenticated');
create policy "staff_interactions"           on interactions           for all using (auth.role() = 'authenticated');

-- Anonymous: insert only for lead + whatsapp capture from website
create policy "anon_insert_leads"            on inbound_leads          for insert with check (true);
create policy "anon_insert_wa_messages"      on whatsapp_messages      for insert with check (true);


-- ============================================================
-- REALTIME
-- Enable live subscriptions for WhatsApp agent dashboard
-- ============================================================
alter publication supabase_realtime add table whatsapp_conversations;
alter publication supabase_realtime add table whatsapp_messages;
alter publication supabase_realtime add table whatsapp_drafts;
