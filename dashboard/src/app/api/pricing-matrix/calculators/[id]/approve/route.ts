 ...
 * bumps the version. Requires a runnable profile and ALL review items resolved.
 */
import { NextRequest, NextResponse } from 'next/server'
import { createClient }              from '@/lib/supabase/server'
import { logActivity }               from '@/lib/log-activity'
import { SB_URL, sbH }               from '@/lib/pm-storage'
import { profileIsRunnable, unresolvedReviewCount } from '@/lib/pm-profile'
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const calc = await fetch(`${SB_URL}/rest/v1/pm_calculators?id=eq.${id}&select=insurer_id,insurer_name,profile,version&limit=1`, { headers: sbH(), cache: 'no-store' })
    ...
    const openItems = unresolvedReviewCount(calc.profile as CellMapProfile)
    if (openItems > 0) return NextResponse.json({ error: `Resolve ${openItems} review item${openItems === 1 ? '' : 's'} before approving` }, { status: 400 })
