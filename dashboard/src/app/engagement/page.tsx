...
function AISummaryStrip({ lead, messages }: { lead: Lead; messages: RealMsg[] }) {
...
function AIDraftPanel({
  ...
  if (sent) return (
    <div style={{ borderTop: '1px solid #e8e8e8', background: '#f9fafb', padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
      <span style={{ fontSize: 12, color: '#15803d' }}>✓ Draft approved</span>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', minWidth: 0, overflow: 'hidden' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        ...
        {/* AI analysis */}
        <AISummaryStrip lead={lead} messages={messages} />

        {/* Email thread body */}
        <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
          {loading && (
            <div style={{ textAlign: 'center', padding: '48px 0', fontSize: 12, color: '#bbb' }}>Loading email thread…</div>
          )}
          {!loading && error && (
            <div style={{ textAlign: 'center', padding: '32px 0', fontSize: 12, color: '#ef4444' }}>{error}</div>
          )}
          {!loading && !error && messages.length === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              {initialMsg && (
                <div style={{ border: '1px solid #e8e8e8', borderRadius: 10, padding: '14px', background: '#fff' }}>
        ...
        {/* AI draft */}
        <AIDraftPanel lead={lead} thread={thread} messages={messages} />
      </div>
  ...
  return (
    <button
      ...
      style={{
        ...
        borderLeft: `3px solid ${isActive ? st.color : 'transparent'}`,
...
export default function EngagementPage() {
  ...
  // Load thread lazily when a lead is selected
  useEffect(() => {
    if (!selectedId) return
    const lead = leads.find(l => l.id === selectedId)
    if (!lead?.email) return
    if (threadMap[selectedId]) return // already loaded
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading ? (
            ...
            <div style={{ padding: '48px 16px', textAlign: 'center' }}>
              ...
              {hasFilters && <button onClick={clearFilters} style={{ fontSize: 11, color: '#3b82f6', background: 'none', border: 'none', cursor: 'pointer' }}>Clear filters</button>}
            </div>
          ) : (
            visible.map(lead => (
              <LeadListItem
