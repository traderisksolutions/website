...
import { UploadCloud, Plus, Download, Send, Loader2 } from 'lucide-react'
            ...
            <tbody>
              ...
              {!loading && filtered.map(r => (
                <tr key={r.id} onClick={() => setOpenId(r.id)} className="border-b border-[--border-subtle] hover:bg-accent/40 cursor-pointer">
                  <td className="px-4 py-2.5 font-medium truncate max-w-[220px]">{r.companies?.name ?? '—'}</td>
...
function DebitNoteDrawer({ id, onClose, onSaved }: { id: string; onClose: () => void; onSaved: () => void }) {
  ...
  async function sendViaEngagement() {
    ...
    try {
      const res = await fetch(`/api/debit-notes/${id}/email-attachment`, { method: 'POST' })
      ...
      openEngagementCompose({
        ...
        body: `Dear ${name || 'Sir/Madam'},\n\nPlease find attached Debit Note ${detail.debit_note_no}.\n\nThank you.`,
        attachment: att,
      })
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center justify-between px-4 pt-3 pb-1">
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={sendViaEngagement} disabled={sending || !detail.contacts?.email}>
