  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 960, margin: '0 auto' }}>
      ...
      {loading ? (
        ...
        <div style={{
          border: '1px dashed var(--border-subtle)', borderRadius: 12, padding: '48px 32px',
          textAlign: 'center', background: 'hsl(var(--muted))',
        }}>
        ...
        <>
            ...
            {members.map(m => {
              ...
              return (
                <button
                  ...
                  style={{
                    border: isActive ? `2px solid ${m.color}` : '2px solid var(--border-subtle)',
                    borderRadius: 10, padding: '14px 16px',
                    background: isActive ? m.color + '08' : 'hsl(var(--card))',
          ...
          {member && (
            <div style={{ border: '1px solid var(--border-subtle)', borderRadius: 12, overflow: 'hidden', background: 'hsl(var(--card))' }}>
              ...
              {/* Panel header */}
              <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: 14, background: member.color + '06' }}>
              ...
              <div style={{ display: 'flex', gap: 10, padding: '16px 20px', borderBottom: '1px solid var(--border-subtle)' }}>
                {[
                  ...
                  <div key={s.label} style={{ background: 'hsl(var(--muted))', border: '1px solid var(--border-subtle)', borderRadius: 8, padding: '12px 16px', flex: 1 }}>
                    ...
                    {feed.map((entry, i) => (
                      <div key={entry.id + i} style={{ display: 'flex', gap: 14, paddingBottom: i < feed.length - 1 ? 18 : 0 }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, width: 32 }}>
                          <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'hsl(var(--muted))', border: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>
                            {TYPE_ICON[entry.type] ?? '📋'}
                          </div>
                          {i < feed.length - 1 && <div style={{ width: 1, flex: 1, background: 'var(--border-subtle)', marginTop: 4 }} />}
                        </div>
