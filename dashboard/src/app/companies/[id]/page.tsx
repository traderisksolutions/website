...
import { NexusSummary } from '@/components/crm/NexusSummary'
...
import { QuotationTab } from '@/components/crm/QuotationTab'
...
import type {
  Company, CompanyAction, CompanyThread, Person, PaymentDerived, PaymentSummary,
  QuoteRow, ActivityEvent, Stage, Alert, LeftOff,
} from '@/lib/crm/types'
...
function CompanyWorkspace() {
  ...
  const { company } = ov
  const proposedCount = ov.actions.filter(a => a.status === 'proposed').length
  ...
  return (
    <div className="min-h-full bg-background">
      <div className="mx-auto max-w-[860px] px-6 py-6">
        ...
        <div className="flex border-b border-[--border-subtle] mb-1 overflow-x-auto -mx-6 px-6 sm:mx-0 sm:px-0" role="tablist">
          {TABS.map(t => {
            const badge = t.key === 'overview' ? ov.alerts.filter(a => a.tone === 'red' || a.tone === 'amber').length
              : t.key === 'threads' ? ov.needsReply
              : t.key === 'quotation' ? (detail?.paymentSummary.overdueCount ?? 0)
              : 0
            return (
              <button
                key={t.key}
                ...
                className={cn('inline-flex items-center gap-1.5 flex-shrink-0 whitespace-nowrap px-3 py-2 border-0 bg-transparent cursor-pointer text-[13px] border-b-2 -mb-px transition-colors',
        ...
        {tab === 'overview' && (
          <>
            ...
            />
            <ActionsPanel companyId={id} actions={ov.actions} onChange={next => setOv(prev => prev ? { ...prev, actions: next } : prev)} />
            <ThreadsPanel threads={ov.lastThreads} compact title="Recent threads" onSeeAll={() => setTab('threads')} />
          </>
        )}
        ...
        {tab === 'policies' && (detail ? <PoliciesPanel policies={detail.policies} /> : <Spinner />)}

        {tab === 'quotation' && (detail && quotes ? <QuotationTab companyId={id} quotes={quotes} notes={detail.payments} summary={detail.paymentSummary} /> : <Spinner />)}

        {tab === 'activity' && (activity ? <CompanyTimeline events={activity} /> : <Spinner />)}

        {proposedCount > 0 && tab !== 'overview' && (
          <p className="mt-4 text-[12px] text-muted-foreground">
            {proposedCount} proposed action{proposedCount === 1 ? '' : 's'} waiting on the{' '}
            <button onClick={() => setTab('overview')} className="text-primary bg-transparent border-0 p-0 cursor-pointer font-semibold hover:underline">Overview</button> tab.
          </p>
        )}
