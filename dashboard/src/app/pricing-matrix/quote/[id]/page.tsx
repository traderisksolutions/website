...
type Quote = {
  id: string; company_name: string | null; effective_date: string | null; member_count: number
  census: CensusMember[]; calculator_ids: string[]; selections: Record<string, Selection>
  results: QuoteResult | null; recommendation: Recommendation | null; priorities: string | null; created_at: string
}
...
export default function QuoteDetailPage() {
  ...
  function startEdit() {
    if (!quote) return
    setEditCompany(quote.company_name ?? '')
  ...
  const liveResult: QuoteResult = useMemo(() => {
    ...
    const insurers: InsurerResult[] = avail.filter(a => editSelected[a.id]).map((a): InsurerResult => {
      ...
      try {
        return computeInsurerQuote(a.id, a.insurer_name, a.effective_date, a.rate_table, editCensus, editSelections[a.id] ?? {}, globals)
      } catch (e) {
  ...
  }, [avail, editSelected, editCensus, editSelections, editEffDate, editNamedCount])
  ...
  async function saveChanges() {
    ...
    const res = await fetch(`/api/pricing-matrix/quote/${id}`, {
      ...
      body: JSON.stringify({ company_name: editCompany, effective_date: editEffDate || null, census: editCensus, calculator_ids: editSelectedIds, selections: editSelections }),
    })
    const d = await safeJson<{ results?: QuoteResult }>(res)
    if (!res.ok || !d.results) { setError(d.error ?? 'Save failed'); setSaving(false); return }
    setQuote(q => q ? {
      ...q, company_name: editCompany || null, effective_date: editEffDate || null, census: editCensus,
      calculator_ids: editSelectedIds, selections: editSelections, results: d.results!, member_count: editNamedCount,
    } : q)
      ...
      {editing ? (
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-3 max-w-lg">
            <input value={editCompany} onChange={e => setEditCompany(e.target.value)} placeholder="Company name" className={inp} />
            <input type="date" value={editEffDate} onChange={e => setEditEffDate(e.target.value)} title="Policy effective date" className={inp} />
          </div>

          <CensusEditor census={editCensus} setCensus={setEditCensus} />
