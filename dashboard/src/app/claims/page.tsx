      ...
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <span style={{
              ...
              padding: '2px 6px', borderRadius: 5, background: `${CLAIM_COLOR}18`, color: CLAIM_COLOR,
            }}>Claims</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
          ...
          <div style={{ position: 'relative' }} ref={menuRef}>
            ...
            >
              {st.label} <ChevronDown size={9} strokeWidth={3} />
            </button>
            {statusMenu && (
              <div style={{
                ...
                {ALL_STATUSES.map(s => {
                  const sc = STATUS[s]
                  return (
                    <button key={s} onClick={e => { e.stopPropagation(); onStatus(lead.id, s); setStatusMenu(false) }}
                      style={{ width: '100%', textAlign: 'left', padding: '8px 12px', fontSize: 12, fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, color: lead.status === s ? sc.color : '#555' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {!loading && newCount > 0 && (
            <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: 'rgba(239,68,68,0.10)', color: '#ef4444' }}>
              {newCount} new
            </span>
          )}
        </div>
      ...
      <main style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', justifyContent: 'center', padding: '80px 0', fontSize: 13, color: '#bbb' }}>No claims</div>
        ) : (
