...
const GEMINI_FLASH    = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent'
...
Return [] for sections with no items; never omit a section`
  ...
  void logGeminiUsage('nexus_synthesis', synthData.usageMetadata ?? {}, caseId, 'gemini-3.5-flash')
...
- reserve_guidance must be null only if there is genuinely insufficient financial data in the threads`
  ...
  if (communicationBriefs.length > 0) {
    try {
      ...
      draftModelName  = 'gemini-3.5-flash'
