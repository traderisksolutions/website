...
const styles = StyleSheet.create({
  ...
  headerCell:  { flex: 1, color: '#fff', fontWeight: 700, textAlign: 'right', paddingRight: 4 },
  headerCellL: { flex: 1, color: '#fff', fontWeight: 700, textAlign: 'left', paddingLeft: 4 },
  ...
  cellR:       { flex: 1, textAlign: 'right', paddingRight: 4 },
  ...
  totalCell:   { flex: 1, textAlign: 'right', paddingRight: 4, fontWeight: 700, color: TOTAL_FG },
...
function TableHeader({ doc, left = 'Coverage' }: { doc: ComparisonDoc; left?: string }) {
  return (
    <View style={styles.headerRow} fixed>
      ...
      {doc.insurer_ids.map(id => <Text key={id} style={styles.headerCellL}>{doc.insurer_names[id]}</Text>)}
    </View>
  )
}

function Rows({ doc, rows, align = 'right' }: { doc: ComparisonDoc; rows: ComparisonRow[]; align?: 'left' | 'right' }) {
  return (
    <>
      {rows.map((r, i) => (
        <View key={i} style={i % 2 === 1 ? [styles.row, styles.rowAlt] : styles.row}>
          <Text style={styles.rowLabel}>{r.label}</Text>
          {doc.insurer_ids.map(id => <Text key={id} style={align === 'right' ? styles.cellR : styles.cellL}>{r.per_insurer[id] ?? ''}</Text>)}
        </View>
      ))}
    </>
  )
