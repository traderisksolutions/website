...
import type { CensusMember, Selection, CategoryOverrides, InsurerResult, QuoteResult } from '@/lib/pm-quote'
...
export async function computeQuoteResultForCalculators(
  ...
  categoryOverrides?: Record<string, CategoryOverrides>,
): Promise<QuoteResult> {
  const inList = calculator_ids.map(i => `"${i}"`).join(',')
  const [calcs, rts, rules] = await Promise.all([
    ...
    // Only APPROVED computation rules ever feed a quote — their lifecycle is independent from the
    // rate table's, so a calculator can be fully priceable (flat lookup) while its translated
    // calculation logic is still in review.
    fetch(`${SB_URL}/rest/v1/pm_computation_rules?calculator_id=in.(${inList})&status=eq.approved&select=calculator_id,rules`, { headers: sbH(), cache: 'no-store' })
      .then(r => (r.ok ? r.json() : [])) as Promise<{ calculator_id: string; rules: RuleStep[] }[]>,
  ])
  const rtByCalc = new Map(rts.map(r => [r.calculator_id, r]))
  const rulesByCalc = new Map(rules.map(r => [r.calculator_id, r.rules]))
  ...
  const insurers: InsurerResult[] = calcs.map((c): InsurerResult => {
    ...
    try {
      return computeInsurerQuote(c.id, name, c.effective_date, rt, census, (selections?.[c.id] ?? {}) as Selection, globals, categoryOverrides?.[c.id], rulesByCalc.get(c.id))
    } catch (e) {
